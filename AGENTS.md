...
**Generated:** 2026-05-04T03:46:35Z
**Commit:** a363e45
**Branch:** develop

## OVERVIEW

Next.js 16 App Router portfolio for an LLM/ML engineer, built on Once UI, MDX case
studies, Cloudflare Workers/OpenNext deployment, GitAura motion widgets, and a
separate Remotion walkthrough-video source tree.

## STRUCTURE

```text
magic-portfolio/
├── src/app/                 # App Router pages, API handlers, metadata
├── src/app/work/projects/   # MDX project case studies
├── src/components/          # Once UI shell, cards, MDX renderers, route guard
├── src/resources/           # Content, theme, routes, icons, project video data
├── remotion/                # Renderable project walkthrough compositions
├── public/images/           # Portfolio, blog, project, OG, avatar assets
├── public/videos/           # Render target for generated walkthrough videos
├── docs/                    # PR review and upstream-sync notes
└── .github/workflows/ci.yml # Type-check, lint, build, Cloudflare deploy
```
...
| Task | Location | Notes |
|------|----------|-------|
| Edit copy, profile, work history | `src/resources/content.tsx` | Main portfolio content source |
| Add/change projects | `src/app/work/projects/*.mdx` | Frontmatter drives cards and detail pages |
| Change routes/theme | `src/resources/once-ui.config.ts` | Style, routes, schema |
| Change project cards/list | `src/components/ProjectCard.tsx` | Carousel card and GitAura preview switch |
| Change MDX rendering | `src/components/mdx.tsx` | Maps Markdown nodes to Once UI components |
| Change Remotion walkthroughs | `remotion/`, `src/resources/projectVisuals.ts` | Shared data |
| Change Cloudflare deploy | `wrangler.jsonc`, `.github/workflows/ci.yml` | OpenNext Worker |

## CODE MAP

| Symbol | Type | Location | Role |
|--------|------|----------|------|
| `getPosts` | function | `src/utils/utils.ts` | Reads MDX frontmatter/content for blog and work |
| `CustomMDX` | component | `src/components/mdx.tsx` | Once UI MDX mapping |
| `RouteGuard` | component | `src/components/RouteGuard.tsx` | Route/password gate |
| `ProjectCard` | component | `src/components/ProjectCard.tsx` | Work card with GitAura preview switch |
| `projectVisuals` | data | `src/resources/projectVisuals.ts` | Remotion storyboard data |
| `PortfolioWalkthroughRoot` | component | `remotion/Root.tsx` | Remotion compositions |
...
- `npm run lint` treats Biome warnings as failures.
- `npm run build` is webpack-backed and also runs Next type validation.
- `npm run type-check` can be affected by stale `.next/dev/types` if a failed dev server
  generated bad route types.
- Remotion package versions should stay aligned; verify with `npx remotion versions`.
- Browser-backed Remotion still/render can fail on sandboxed macOS with Chromium
  `bootstrap_check_in ... Permission denied`; bundle/version checks are not render proof.
